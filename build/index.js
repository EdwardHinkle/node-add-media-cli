...
var MovieDB = require('moviedb')(config.moviedb_key);
var defaultWatchStatus = "finished";
...
if (searchType == "movie") {
    MovieDB.searchMovie({ query: searchTitle }, function (err, res) {
        var movieResult = res.results[0];
        MovieDB.movieInfo({ id: movieResult.id }, function (err, movieInfo) {
            saveMovieTemplate({
    ...
    MovieDB.searchTv({ query: searchTitle }, function (err, res) {
        if (err) {
            console.log("Error");
            console.log(err);
        }
        ...
        if (seasonId == undefined) {
            MovieDB.tvInfo({ id: showResult.id }, function (err, show) {
            ...
            MovieDB.tvSeasonInfo({ id: showResult.id, season_number: seasonId }, function (err, episodeInfo) {
                _.each(episodeInfo.episodes, function (episode) {
                    console.log("Episode " + episode.episode_number + ": " + episode.name + " (" + moment(episode.air_date, "YYYY-MM-DD").format("MMM D, Y") + ")");
                });
            });
        }
        else {
            MovieDB.tvInfo({ id: showResult.id }, function (err, showInfo) {
                MovieDB.tvExternalIds({ id: showResult.id }, function (err, showIds) {
                    MovieDB.tvSeasonInfo({ id: showResult.id, season_number: seasonId }, function (err, seasonInfo) {
                        MovieDB.tvEpisodeInfo({ id: showResult.id, season_number: seasonId, episode_number: episodeId }, function (err, episodeInfo) {
                            saveTvTemplate({
                                postCategory: postCategory,
                                watchStatus: watchStatus,
